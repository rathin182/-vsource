...
export async function GET(req: NextRequest) {
  try {
    ...
    let counselorId = sp.get("counselorId") ?? undefined;
    if (userRole.toUpperCase() === "COUNSELLOR") {
      counselorId = userId;
    }
    ...
    const leadStage = sp.get("leadStage") as LeadStage | null;
...
export async function POST(req: NextRequest) {
  try {
    ...
    const body = LeadCreateSchema.parse({
      ...
      ieltsScore:
        payload.englishTestType === "IELTS"
          ? payload.listeningScore
            ? (Number(payload.listeningScore) +
                Number(payload.readingScore) +
                Number(payload.writingScore) +
                Number(payload.speakingScore)) /
              4
            : undefined
          : undefined,
      pteScore:
        payload.englishTestType === "PTE"
          ? payload.pteScore || undefined
          : undefined,
      toeflScore:
        payload.englishTestType === "TOEFL"
          ? payload.toeflScore || undefined
          : undefined,
      duolingoScore:
        payload.englishTestType === "DUOLINGO"
          ? payload.duolingoScore || undefined
          : undefined,
      ...
      preferredTiers: Array.isArray(payload.preferredTiers)
        ? payload.preferredTiers
        : [],
